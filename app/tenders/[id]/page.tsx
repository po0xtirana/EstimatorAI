...
type Document = { id: string; file_name: string; document_type: string; processing_status: string; source_url: string | null };
...
export default function TenderDetailPage() {
  ...
  async function addDocument(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    setBusy(true);
    const response = await fetch(`/api/tenders/${id}/documents`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ fileName: documentName, documentType, extractedText: documentText, tradeProfileId: profileId || null }) });
    const data = await response.json();
    setMessage(response.ok ? "Tender document registered and scope quantities extracted." : data.error);
    if (response.ok) { setDocumentName(""); setDocumentText(""); await load(); }
    setBusy(false);
  }
  ...
  return <main className="empty-page">
    ...
    <div className="detail-grid">
      <section className="profile-card"><div className="profile-card-heading"><div><h2>Tender package</h2><p className="muted">Upload a text-readable tender file or paste extracted text. PDFs and drawings still need text extraction or manual review before registration.</p></div></div>{documents.map((document) => <div className="input-row" key={document.id}><div><span>{document.file_name}</span><small>{document.document_type} - {document.processing_status}</small></div></div>)}<form className="stack-form" onSubmit={addDocument}><input type="file" accept=".txt,.md,.csv,.json,text/plain,text/csv,application/json" onChange={loadTextFile} /><input value={documentName} onChange={(event) => setDocumentName(event.target.value)} placeholder="Document name, e.g. Scope.txt" required /><select value={documentType} onChange={(event) => setDocumentType(event.target.value)}><option value="specification">Specification</option><option value="drawing">Drawing</option><option value="bill_of_quantities">Bill of quantities</option><option value="addendum">Addendum</option><option value="schedule">Schedule</option></select><textarea value={documentText} onChange={(event) => setDocumentText(event.target.value)} placeholder="Paste extracted text or notes from the document for evidence capture" rows={4} /><button className="text-button" type="submit" disabled={busy}>Register document -&gt;</button></form></section>
