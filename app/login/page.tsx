...
export default function LoginPage() {
  ...
  return <main className="auth-page"><div className="auth-card"><div className="brand auth-brand"><span className="brand-mark">E</span><span>EstimatorAI</span></div><p className="eyebrow accent">Private contractor workspace</p><h1>Sign in to EstimatorAI</h1><p className="auth-copy">Use your work email to receive a secure sign-in link.</p><form onSubmit={submit}><label htmlFor="email">Work email</label><input id="email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} required autoComplete="email" placeholder="you@company.ca" /><button className="button auth-button" type="submit" disabled={busy}>{busy ? "Sending..." : "Email me a sign-in link"}<span>→</span></button></form>{message && <p className="auth-message" role="status">{message}</p>}</div></main>;
}
