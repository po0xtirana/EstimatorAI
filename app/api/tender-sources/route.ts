...
export async function GET() {
  try {
    ...
    const [sources, connections, runs] = await Promise.all([
      ...
      admin.from("organization_tender_source_connections").select("source_key,status,connection_type,forwarding_address,last_success_at,last_error").eq("organization_id", ctx.organizationId),
    ...
    return NextResponse.json({ sources: result, summary: { connected: result.filter((source: any) => source.connection_status === "connected" || source.organizationConnection?.status === "connected").length, available: result.length, healthy: result.filter((source: any) => source.connection_status === "connected" && !source.last_error).length } });
  } catch (error) {
